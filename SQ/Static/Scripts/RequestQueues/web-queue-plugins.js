...
(function ($) {
	var tmp, loading, overlay, wrap, outer, content, close, title, nav_left, nav_right,
		...
		isMSIE = ($('html').hasClass('ie7') || $('html').hasClass('ie8')),
		...
		_show = function () {
			...
			var wraptop = parseInt(Math.max(view[3] - 20, view[3] + ((view[1] - content.height()) * 0.5) - currentOpts.padding));

			if (typeof currentOpts['min-top'] == 'number') {
				if (wraptop < currentOpts['min-top']) {
					wraptop = currentOpts['min-top'];
				}
			}

			if (typeof currentOpts['top'] == 'number') {
				wraptop = currentOpts['top'];
			}

			wrap.css({
				'top': wraptop,
				'left': parseInt(Math.max(view[2] - 20, view[2] + ((view[0] - content.width()) * 0.5) - currentOpts.padding))
			});
		},
		...
		_finish = function () {
			...
			if (currentOpts.type == 'iframe') {
				$('<iframe id="fancybox-frame" name="fancybox-frame' + new Date().getTime() + '" frameborder="0" hspace="0" ' + (isMSIE ? 'allowtransparency="true""' : '') + ' scrolling="' + selectedOpts.scrolling + '" src="' + currentOpts.href + '"></iframe>').appendTo(content);
			}
		...
		_get_zoom_to = function () {
			...
			//Modified by Edward
			//Added 20px to 2nd attribute of Max.max()
			to.top = parseInt(Math.max(view[3] - 20, 20 + view[3] + ((view[1] - to.height - 40) * 0.5)), 10);
			to.left = parseInt(Math.max(view[2] - 20, 20 + view[2] + ((view[0] - to.width - 40) * 0.5)), 10);
...
/**
 * .dotdotdot from jquery.plugin.js
 */
(function (j, d) {
	if (j.fn.dotdotdot) {
		return
	...
	function o(s, t, y, x, w) {
		...
		s.contents().detach().each(function () {
			var A = this, z = j(A);
			if (typeof A == "undefined" || (A.nodeType == 3 && j.trim(A.data).length == 0)) {
				return true
			} else {
				if (z.is(u)) {
					s.append(z)

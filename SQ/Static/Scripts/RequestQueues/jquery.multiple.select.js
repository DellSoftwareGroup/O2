(function ($) {
	...
	MultipleSelect.prototype = {
		constructor: MultipleSelect, init: function () {
			...
			var that = this, $elm = $(elm), multiple = this.options.multiple, optAttributesToCopy = ["class", "title"], clss = $.map(optAttributesToCopy, function (att, i) {
			...
			if ($elm.is("option")) {
				var value = $elm.val(), text = that.options.textTemplate($elm), selected = $elm.prop("selected"), style = this.options.styler(value) ? ' style="' + this.options.styler(value) + '"' : "";
				disabled = groupDisabled || $elm.prop("disabled");
				if (this.options.blockSeparator > "" && this.options.blockSeparator == $elm.val()) {
					var li = $("<li" + clss + style + ">", '<label class="' + this.options.blockSeparator + (disabled ? "disabled" : "") + '">', "</label>", "</li>");
					li.find("label").append(document.createTextNode(text));
					return li
				} else {
					var li = $("<li" + clss + style + ">" + "<label" + (disabled ? ' class="disabled"' : "") + ">" + '<input type="' + type + '" ' + this.selectItemName + (selected ? ' checked="checked"' : "") + (disabled ? ' disabled="disabled"' : "") + (group ? ' data-group="' + group + '"' : "") + "/> " + "</label>" + "</li>");
				...
				var group = $("<div/>");
				group.append('<li class="group">' + '<label class="optgroup' + (disabled ? " disabled" : "") + '" data-group="' + _group + '">' + (this.options.hideOptgroupCheckboxes ? "" : '<input type="checkbox" ' + this.selectGroupName + (disabled ? ' disabled="disabled"' : "") + " /> ") + label + "</label>" + "</li>");
				li.find("label").append(document.createTextNode(text));
	...
	$.fn.multipleSelect = function () {
		var option = arguments[0], args = arguments, value, allowedMethods = ["getSelects", "setSelects", "enable", "disable", "checkAll", "uncheckAll", "focus", "blur", "refresh", "close"];
		this.each(function () {

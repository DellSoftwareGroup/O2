var singleQueueTables = function ($) {
	var commonStyles = function () {
		...
		function tempStyles() {
			var taggedCol = $(".title-col"), label = ['<span class="icon urgent">Urgent</span>', '<span class="icon new">New</span>', '<span class="icon legacy">Legacy</span>', '<span class="icon legacy">Legacy</span>'];
		...
		function GridRequest_DataBound() {
			var PagerConfig = {
				...
				messages: {display: '<li class="">{0} - {1} of {2}</li>'}
			};
	...
	var ownerColorTag = function () {
		var colorToName = [{name: "James Gomez", color: "#6ea204"}, {
			name: "Michael Hughes",
			color: "#42aeaf"
		...
		$(".owner-col").each(function () {
			...
			colorToName.forEach(function (owner) {
				if (owner.name == ownerName) {
					$that.find("span:first-child").css("background-color", owner.color).addClass("color-id")
				}
			})
		})
	};
	var statusIcon = function () {
		var icons = [{status: "In Progress", "class": "started-icon"}, {
			status: "Complete",
			"class": "done-icon"
		}, {status: "Pending", "class": "backlog-icon"}];
		$(".status-col").each(function () {
			var status = $(this).text(), iconPlaceholder = $(this).find("i");
		...
		function popoverBuilder(requestId) {
			...
			if (Array.isArray(filteredTasks)) {
				filteredTasks.forEach(function (task) {
					trTasks += String() + '<tr><td><div class="tsk-group">' + task.group + "</div></td>" + '<td><div class="tsk-task">' + task.task + "</div></td>" + '<td><div class="tsk-owner">' + task.owner + "</div></td>" + '<td><div class="tsk-owner">' + task.status + "</div></td>" + '<td><div class="pln-start">' + task.plannedStart + "</div></td>" + '<td><div class="pln-end">' + task.plannedEnd + "</div></td>" + '<td><div class="actual-start">' + task.actualStart + "</div></td>" + '<td><div class="actual-end">' + task.actualEnd + "</div></td></tr>"
				});
				var tasksTmpl = String() + '<div class="popover sq-popover" role="tooltip">' + '<h3 class="popover-title">Group Tasks</h3>' + "<div>" + "<table><thead>" + "<tr><th>Group</th><th>Task</th><th>Owner</th><th>Status</th><th>Planned Start</th><th>Planned End</th><th>Actual Start</th><th>Actual End</th></tr>" + "</thead>" + '<tbody class="popover-content">' + "</tbody></table>" + "</div></div>";
				var popUp = {tmpl: tasksTmpl, content: trTasks};
				return popUp
			} else {
				var tasksNotFound = String() + '<div class="popover popover-nr" role="tooltip">' + '<h3 class="popover-title">Group Tasks</h3>' + '<div class="popover-content"></div>' + "</div>";
				var noTaskResp = {tmpl: tasksNotFound, content: filteredTasks};
				return noTaskResp
			}
		}
	...
	return {requestListings: requestListings, taskListings: taskListings, editTaskListing: editTaskListing}
}(jQuery);